...
export function EditableContactLeadForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-[2rem] border border-white/12 bg-white/8 p-6 shadow-2xl shadow-black/25 backdrop-blur-xl md:p-8">
      ...
      {message ? (
        <div className={`mt-5 flex items-start gap-3 rounded-2xl px-4 py-3 text-sm font-bold ${status === 'success' ? 'bg-emerald-400/15 text-emerald-200' : 'bg-rose-400/15 text-rose-200'}`}>
...
function Field({ name, label, type = 'text', placeholder, required = false }: { name: string; label: string; type?: string; placeholder?: string; required?: boolean }) {
  return (
    <label className="grid gap-2 text-sm font-black text-white/78">
      {label}
      <input name={name} type={type} required={required} placeholder={placeholder} className="h-12 rounded-2xl border border-white/12 bg-white/8 px-4 text-base font-medium text-white outline-none transition placeholder:text-white/35 focus:border-[var(--slot4-accent)]" />
    </label>
  )
}
