...
const inputClass = 'h-12 rounded-2xl border border-white/12 bg-white/8 px-4 text-base font-bold text-white outline-none transition placeholder:text-white/35 focus:border-[var(--slot4-accent)] focus:bg-white/10'
const buttonClass = 'inline-flex h-12 items-center justify-center rounded-2xl bg-[linear-gradient(135deg,#6fd1d7,#5df8d8)] px-6 text-sm font-black uppercase tracking-[0.18em] text-[#082033] shadow-sm transition hover:-translate-y-0.5 disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-2xl px-4 py-3 text-sm font-bold ${status === 'success' ? 'bg-emerald-400/15 text-emerald-200' : 'bg-rose-400/15 text-rose-200'}`}>{message}</p> : null}
