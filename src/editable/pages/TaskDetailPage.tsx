...
import { ArrowLeft, Building2, Camera, CheckCircle2, Download, ExternalLink, FileText, Mail, MapPin, MessageSquare, Phone, Tag, UserRound } from 'lucide-react'
...
const stripTags = (value: string) => value.replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/&nbsp;/g, ' ').replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim()
...
export function TaskDetailView({ task, post, related, comments = [] }: { task: TaskKey; post: SitePost; related: SitePost[]; comments?: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  const detailVars = {
    '--detail-bg': '#061826',
    '--detail-text': '#f5fbff',
    '--detail-surface': '#0b2434',
    '--detail-accent': '#6fd1d7',
  } as CSSProperties
  ...
  return (
    <section className="mx-auto grid max-w-[1440px] gap-8 px-4 py-14 sm:px-6 sm:py-16 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 lg:py-20">
      <article className="min-w-0 rounded-[2.6rem] border border-white/12 bg-white/6 p-5 shadow-[0_24px_80px_rgba(0,0,0,0.24)] sm:p-8 lg:p-12">
        ...
        <h1 className="mt-4 max-w-4xl text-4xl font-semibold leading-[0.96] tracking-[-0.08em] sm:text-5xl lg:text-6xl">{post.title}</h1>
        <p className="mt-5 max-w-3xl text-base leading-8 text-white/70">{summaryText(post)}</p>
        {heroImage ? <img src={heroImage} alt={post.title} className="mt-8 w-full rounded-[2rem] object-cover shadow-[0_20px_70px_rgba(0,0,0,0.24)]" /> : null}
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_380px]">
        <article className="rounded-[2.7rem] border border-white/12 bg-white/6 p-6 shadow-[0_24px_80px_rgba(0,0,0,0.24)] sm:p-9">
          <div className="grid gap-6 sm:grid-cols-[140px_1fr]">
            <div className="flex h-36 w-36 items-center justify-center overflow-hidden rounded-[2rem] border border-white/12 bg-white/8">
            ...
            <div>
              ...
              <h1 className="mt-3 max-w-4xl text-4xl font-semibold leading-[0.96] tracking-[-0.08em] sm:text-5xl">{post.title}</h1>
              <p className="mt-5 max-w-3xl text-base leading-8 text-white/70">{summaryText(post)}</p>
            </div>
          </div>
          <div className="mt-8 grid gap-3 sm:grid-cols-2">
            {address ? <DetailMetaChip label="Location" value={address} /> : null}
            {phone ? <DetailMetaChip label="Phone" value={phone} /> : null}
            {email ? <DetailMetaChip label="Email" value={email} /> : null}
            {website ? <DetailMetaChip label="Website" value={website.replace(/^https?:\/\//, '')} /> : null}
          </div>
          <BodyContent post={post} />
  ...
  return (
    <section className="mx-auto grid max-w-[1440px] gap-7 px-4 py-14 sm:px-6 sm:py-16 lg:grid-cols-[0.88fr_1.12fr] lg:px-8 lg:py-20">
      <aside className="rounded-[2.5rem] border border-white/12 bg-[linear-gradient(135deg,rgba(9,60,93,0.96),rgba(59,117,151,0.9))] p-7 text-white shadow-[0_24px_80px_rgba(0,0,0,0.24)] lg:sticky lg:top-24 lg:self-start">
        ...
        <h1 className="mt-4 max-w-2xl text-4xl font-semibold leading-[0.96] tracking-[-0.08em] sm:text-5xl">{post.title}</h1>
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          {phone ? <a href={`tel:${phone}`} className="rounded-full bg-white px-5 py-3 text-sm font-black text-[#082033]">Call now</a> : null}
    ...
    <section className="mx-auto max-w-[1440px] px-4 py-14 sm:px-6 sm:py-16 lg:px-8 lg:py-20">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[0.72fr_1.28fr]">
        <aside className="rounded-[2.5rem] border border-white/12 bg-white/6 p-7 shadow-[0_24px_80px_rgba(0,0,0,0.24)] lg:sticky lg:top-24 lg:self-start">
          <div className="inline-flex items-center gap-2 rounded-full bg-[linear-gradient(135deg,#6fd1d7,#5df8d8)] px-4 py-2 text-xs font-black uppercase tracking-[0.2em] text-[#082033]">
            <Camera className="h-4 w-4" />
            Image story
          </div>
          <h1 className="mt-6 max-w-2xl text-4xl font-semibold leading-[0.96] tracking-[-0.08em] sm:text-5xl">{post.title}</h1>
        ...
        <div className="columns-1 gap-5 space-y-5 md:columns-2">
          {(images.length ? images : ['/placeholder.svg?height=900&width=1200']).map((image, index) => (
            <figure key={`${image}-${index}`} className="break-inside-avoid overflow-hidden rounded-[2rem] border border-white/12 bg-white/6 shadow-[0_18px_60px_rgba(0,0,0,0.2)]">
              <img src={image} alt="" className="w-full object-cover" />
              {index === 0 ? <figcaption className="p-5 text-sm font-semibold text-white/65">Featured visual from this post.</figcaption> : null}
            </figure>
          ))}
        </div>
      </div>
    ...
    <section className="relative mx-auto max-w-[1120px] px-4 py-14 sm:px-6 sm:py-16 lg:px-8 lg:py-20">
      ...
      <div className="mt-8 rounded-[2.8rem] border border-white/12 bg-[linear-gradient(180deg,rgba(255,255,255,0.08),rgba(255,255,255,0.05))] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.24)] sm:p-8 lg:p-10">
        <div className="max-w-3xl">
          <p className="text-[11px] font-black uppercase tracking-[0.28em] text-[var(--detail-accent)]">Saved bookmark</p>
          <h1 className="mt-4 text-4xl font-semibold leading-[0.94] tracking-[-0.08em] sm:text-5xl lg:text-6xl">
            {post.title}
          </h1>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <span className="inline-flex items-center gap-2 rounded-full border border-white/12 bg-white/8 px-4 py-2 text-xs font-black uppercase tracking-[0.22em] text-white/75">
              {category}
            </span>
          </div>
        </div>
    ...
    <section className="mx-auto grid max-w-[1440px] gap-8 px-4 py-14 sm:px-6 sm:py-16 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 lg:py-20">
      <article className="rounded-[2.7rem] border border-white/12 bg-white/6 p-6 shadow-[0_24px_80px_rgba(0,0,0,0.24)] sm:p-9">
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-[120px_1fr]">
          <div className="flex h-28 w-28 items-center justify-center rounded-[1.8rem] bg-[linear-gradient(135deg,#6fd1d7,#5df8d8)] text-[#082033]">
        ...
        <BodyContent post={post} />
        {fileUrl ? (
          <div className="mt-8 overflow-hidden rounded-[2rem] border border-white/12 bg-white/6">
            <div className="flex items-center justify-between gap-3 border-b border-white/12 bg-white/8 p-4">
              <span className="text-sm font-black">Document preview</span>
              <Link href={fileUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full bg-[linear-gradient(135deg,#6fd1d7,#5df8d8)] px-4 py-2 text-xs font-black text-[#082033]">Download <Download className="h-4 w-4" /></Link>
            </div>
  ...
  return (
    <section className="mx-auto grid max-w-[1440px] gap-8 px-4 py-14 sm:px-6 sm:py-16 lg:grid-cols-[420px_minmax(0,1fr)] lg:px-8 lg:py-20">
      <aside className="rounded-[2.7rem] border border-white/12 bg-white/6 p-8 text-center shadow-[0_24px_80px_rgba(0,0,0,0.24)] lg:sticky lg:top-24 lg:self-start">
        ...
        <h1 className="mt-6 text-4xl font-semibold leading-[0.96] tracking-[-0.08em]">{post.title}</h1>
        {role ? <p className="mt-3 text-[11px] font-black uppercase tracking-[0.18em] text-[var(--detail-accent)]">{role}</p> : null}
        <ContactAction website={website} email={email} />
      </aside>
      <article className="rounded-[2.7rem] border border-white/12 bg-white/6 p-7 shadow-[0_24px_80px_rgba(0,0,0,0.24)] sm:p-10">
        <BodyContent post={post} />
...
function BodyContent({ post, compact = false }: { post: SitePost; compact?: boolean }) {
  return <div className={`article-content mt-8 max-w-none ${compact ? 'text-base leading-8' : 'text-lg leading-9'} text-white/82`} dangerouslySetInnerHTML={{ __html: formatPlainText(getBody(post)) }} />
}
  ...
  return (
    <section className="mt-8">
      ...
      <div className={`mt-4 grid gap-3 ${large ? 'sm:grid-cols-2' : 'grid-cols-2 sm:grid-cols-4'}`}>
        {images.slice(0, large ? 4 : 8).map((image, index) => <img key={`${image}-${index}`} src={image} alt="" className="aspect-[4/3] rounded-[1.4rem] object-cover ring-1 ring-white/12" />)}
      </div>
    </section>
  )
}
...
function MapBox({ src, label }: { src: string; label: string }) {
  return (
    <div className="overflow-hidden rounded-[2rem] border border-white/12 bg-white/6 shadow-[0_24px_80px_rgba(0,0,0,0.2)]">
...
function ContactAction({ website, phone, email }: { website?: string; phone?: string; email?: string }) {
  ...
  return (
    <div className="mt-5 rounded-[2rem] border border-white/12 bg-white/6 p-5 text-left">
      <p className="text-[11px] font-black uppercase tracking-[0.22em] text-white/45">Quick actions</p>
      <div className="mt-4 flex flex-wrap gap-3">
        {website ? <Link href={website} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full bg-[linear-gradient(135deg,#6fd1d7,#5df8d8)] px-4 py-2 text-sm font-black text-[#082033]">Website <ExternalLink className="h-4 w-4" /></Link> : null}
        {phone ? <a href={`tel:${phone}`} className="inline-flex items-center gap-2 rounded-full border border-white/12 px-4 py-2 text-sm font-black text-white"><Phone className="h-4 w-4" /> Call</a> : null}
        {email ? <a href={`mailto:${email}`} className="inline-flex items-center gap-2 rounded-full border border-white/12 px-4 py-2 text-sm font-black text-white"><Mail className="h-4 w-4" /> Email</a> : null}
      </div>
    </div>
  )
}
...
function BadgeLine({ label, value }: { label: string; value: string }) {
  return <div className="flex items-center justify-between gap-4 rounded-2xl border border-white/12 bg-white/8 px-4 py-3 text-sm"><span className="font-black uppercase tracking-[0.16em] text-white/45">{label}</span><span className="font-black text-white">{value}</span></div>
}
    ...
    <aside className="min-w-0 space-y-5">
      {!compact ? (
        <div className="rounded-[2rem] border border-white/12 bg-white/6 p-5">
          <p className="text-[11px] font-black uppercase tracking-[0.22em] text-white/45">About this post</p>
      ...
      {related.length ? (
        <div className="rounded-[2rem] border border-white/12 bg-white/6 p-5">
          <div className="flex items-center justify-between gap-3">
            ...
            <Link href={taskConfig?.route || '/'} className="text-xs font-black uppercase tracking-[0.16em] text-[var(--detail-accent)]">View all</Link>
          </div>
...
function RelatedCard({ task, post }: { task: TaskKey; post: SitePost }) {
  ...
  if (task === 'sbm') {
    return <BookmarkCard post={post} href={href} />
  }
  if (task === 'image') {
    return <ImageFirstCard post={post} href={href} index={0} />
  }
  if (task === 'listing') {
    return <ArticleListCard post={post} href={href} index={0} />
  }
  if (task === 'pdf' || task === 'profile') {
...
function EditableComments({ slug, comments }: { slug: string; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  return (
    <section className="mt-10 rounded-[2rem] border border-white/12 bg-white/6 p-5">
      <div className="flex items-center gap-2 text-lg font-black"><MessageSquare className="h-5 w-5" /> Comments</div>
      <div className="mt-5 grid gap-3">
        {comments.slice(0, 5).map((comment) => (
          <div key={comment.id} className="rounded-2xl border border-white/12 bg-white/8 p-4">
        ...
        {!comments.length ? <p className="text-sm text-white/60">No comments yet for {slug}.</p> : null}
      </div>
    </section>
  )
}
